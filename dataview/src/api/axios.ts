...
axiosInstance.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    if (includes(fetchAllowList, url)) return config
    // 获取 token
    ...
    config.headers["serviceId"]= localStorage.getItem('serviceId')||undefined;
    return config
  },
  (err: AxiosError) => {
    Promise.reject(err)
...
axiosInstance.interceptors.response.use(
  ...
  (err: AxiosError) => {
    ...
    switch (status) {
      case 401:
        if (err.response&&err.response.data&&JSON.stringify(err.response.data).indexOf('演示环境,当前帐号没有开启此功能权限')) {
          window['$message'].error('演示环境,当前帐号没有开启此功能权限')
          return;
        }
        routerTurnByName(PageEnum.BASE_LOGIN_NAME)
        Promise.reject(err)

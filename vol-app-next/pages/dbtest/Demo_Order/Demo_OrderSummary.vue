...
<template>
	<vol-view ref="viewRef" :table="table" :columns="columns" :table-data="tableData"
		...
		<!--表格配置 -->
		<vol-alert>生成页面显示合计信息</vol-alert>
		<view class="summary">
			<text class="txt">总价格：</text><text class="vol-tag vol-tag-primary">{{summary.TotalPrice}}元</text>
			<text class="txt">总数量：</text><text class="vol-tag vol-tag-error">{{summary.TotalQty}}箱</text>
		</view>
...
<script setup>
	...
	const searchAfter = (res) => {
		summary.value.TotalPrice = res.summary.TotalPrice || 0;
		summary.value.TotalQty = res.summary.TotalQty || 0;
		nextTick(() => {

...
<template>
	<vol-view ref="viewRef" :table="table" :columns="columns" :table-data="tableData"
		...
		<vol-alert>点击添加或者行数据进入一对多编辑</vol-alert>
		<!--表格配置 -->
		<vol-table ref="tableRef" :ck="false" :index="false" :url="tableUrl" @rowClick="modelOpenBefore"
			:loadBefore="searchBefore" :loadAfter="searchAfter" :direction="direction" titleField="ProductCode"
...
<script setup>
	...
	const modelOpenBefore = (row, index, obj, callback) => {
		//跳转到新页面编辑
		uni.navigateTo({
			url: "/pages/dbtest/Demo_Product/Demo_ProductEdit?id=" + ((row || {})[table.key] || ''),
			fail(e) {
				console.log(e)
			}
		})

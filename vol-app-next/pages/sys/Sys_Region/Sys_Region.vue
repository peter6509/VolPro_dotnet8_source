...
<template>
	<vol-view ref="viewRef" :table="table" :columns="columns" :table-data="tableData"
		...
		<!--表格配置 -->
		<vol-table ref="tableRef" :ck="false" :index="false" :url="tableUrl" @rowClick="modelOpenBefore" :loadBefore="searchBefore"
...
<script setup>
	...
	const modelOpenBefore = (row, index, obj, callback) => {
        //跳转到新页面编辑
        uni.navigateTo({
			url: "/pages/sys/Sys_Region/Sys_RegionEdit?id=" + ((row || {})[table.key] || ''),
            fail(e) {
                console.log(e)
            }
		})

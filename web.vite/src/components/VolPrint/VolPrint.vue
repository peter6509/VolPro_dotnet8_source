      ...
      <div v-if="!view" class="paper">
        ...
        <el-popover placement="bottom" :width="400" trigger="click">
          ...
          <div class="custom-popover">
            <div class="custom-popover-text">設置纸張寬高(mm)</div>
            <div class="flex-row mt-10">
              <el-input v-model="paperWidth" :min="1" placeholder="寬(mm)" />
...
/**
 * 這裡必須要在 onMounted 中去構建 左側可拖拽元素 或者 設計器
...
const buildDesigner = (ops) => {
  ...
  hiprintTemplate = newHiprintPrintTemplate(TEMPLATE_KEY, {
    ...
    onDataChanged: (type, json) => {
      console.log(type); // 新增、移動、删除、修改(參數調整)、大小、旋轉
...
const printCallback = (status) => {
  ...
  //     callbackApi=callbackApi+"?options="+(router.query.options||'')
  // }else{
  //     callbackApi=callbackApi+"&options="+(router.query.options||'')
  // }
...
/**
 * 導出模板 json tid
 * 僅導出 options, 不導出 printElementType
 * 必須確保 hiprintTemplate 已成功創建
 */
const exportJsonTid = () => {
...
const addTemplate = async (ops, table) => {
  ...
  //     return proxy.$message.error('未配置打印,請在後台startup.cs中方法ConfigureContainer配置打印參數')
  // }
  // if (!row) {
  //     return;
  // }

  ...
  >
    ...
    >
      ...
      <el-radio-group v-show="hasFlow" style="padding-left: 15px" v-model="activeName" class="ml-4">
        <el-radio label="audit" size="large">{{ $ts('審核') }}</el-radio>
        <el-radio label="log" size="large">{{ $ts('審核記錄') }}</el-radio>
      </el-radio-group>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const columns = reactive([
      ...
      { title: '審批结果', field: 'auditStatus', width: 70, bind: { data: [] } },

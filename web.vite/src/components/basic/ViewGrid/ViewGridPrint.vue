<template>
  <vol-box :footer="false" v-model="model" :height="height" :width="width" :padding="0" :lazy="true" title="選擇打印模板">
    ...
    <template #footer>
      <div class="btns">
        <el-button type="default" plain size="small" @click="model = false">{{ $ts('關閉') }}</el-button>
...
const confirm = (item) => {
  let url = `${location.origin}/#/print?templateId=${item.id}&ids=${_ids.join(',')}&table=${_table}&view=1&options=`+(_options||'')

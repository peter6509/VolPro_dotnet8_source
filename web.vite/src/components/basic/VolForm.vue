        ...
        >
          ...
          <div v-else-if="isReadonlyImgFile(item, formFields)">
            ...
            <div
              ...
              :key="fileIndex"
            >
              <a @click="dowloadFile(formFields[item.field][fileIndex], $store, http)">{{
                file.name
              }}</a>
            </div>
          </div>
          ...
          <div v-else :class="{ 'form-item-extra': item.extra }">
            ...
            <template v-else-if="['select', 'selectList'].indexOf(item.type) != -1">
              <el-select-v2
                ...
                :placeholder="$ts(item.placeholder || '請選擇')"
            ...
            <el-input
              ...
              :placeholder="$ts(item.placeholder || item.title)"
            />
            <!-- 2021.11.18修複el-input没有默認enter事件時回車異常 -->
            <el-input
              :size="size"
              clearable
              :ref="item.field"
...
const fileOnChange = (files, item) => {
  formRef.value.clearValidate(item.field)
  if (item.onChange) {
...
const validate = async (callback) => {
  ...
  await formRef.value.validate((valid) => {
    if (!valid) {
      proxy.$message.error(proxy.$ts('數據驗證未通過!'))
      result = false
      return
    }
    if (typeof callback === 'function') {
      try {
        ...
        let msg = `表單驗證回調方法異常：${error.message}`

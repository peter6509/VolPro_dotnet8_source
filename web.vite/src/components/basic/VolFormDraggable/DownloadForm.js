...
export default function () {
    ...
    let _formOptions = this.options.formOptions.map(m => {
        let _op = m.map((m1, i) => {
            ...
            if (m1.type=='line') {
               ...
               let name=  this.currentComponents.find(c=>{return c.field==m1.field}).name
                _obj=`
                {
                    colSize: 12,
                    render: (h) => {
                      return  <div
                          ...
                          <div style="padding-left: 1px; font-weight: bold; font-size: 13px;">{this.$ts('${name}')}</div>
                        </div>
                    }
                  }
                `
            ...
            else if (m1.data && m1.data.length && !m1.dataKey) {
  ...
  <el-tabs v-model="tabsModel"
    ...
    if ('download' in document.createElement('a')) { // 非IE下載
        ...
        elink.click()
        URL.revokeObjectURL(elink.href) // 释放URL 對象

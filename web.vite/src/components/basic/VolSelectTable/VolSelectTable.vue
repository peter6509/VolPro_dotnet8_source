  ...
  <div ref="selectRef" class="input-select content">
    ...
    >
      ...
      <div
        ...
        @click.stop
        :style="{
          height: height + (paginationHide ? 0 : 37) + (searchForm.length ? 45 : 0) + 'px'
        }"
      >
        <div class="input-search" v-if="searchForm.length || !single">
          ...
          <el-button @click="searchClick" type="primary" v-if="searchForm.length" plain
            ><i class="el-icon-search"></i>{{ $ts('查詢') }}</el-button
          >
          <el-button
            v-if="!single"
...
export default {
  ...
  setup(props, context) {
    ...
    const inputClick = () => {
      if (!props.url) {
        proxy.$message.error('未配置url屬性,見文檔:http://doc.volcore.xyz/example/selectTable.html')
        return
      }
      currentIndex = -1
      if (!show.value) {
    ...
    const getSelectData=(item)=>{
        return props.columns.find(x=>{return x.field==item.field}).bind.data;
    }

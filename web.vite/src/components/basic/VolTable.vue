...
export default defineComponent({
  ...
  props: {
    ...
    textInline: {
      // 表格内容超出後是否換行顯示（2020.01.16）
      type: Boolean,
      default: true,
    },
    tableData: {
      // 表數據源,配置了url就不用傳這個參數了
      type: Array,
      default: () => {
        return [];
      },
    ...
    index: {
      // 是否創建索引號,如果需要表格編輯功能，這裡需要設置為true
      type: Boolean,
      default: false,
    },
    allowEmpty: {
    ...
    reserveSelection: {
      //分頁或者刷新表格數據後是否保留複選框選擇狀態，2024.09.10
      type: Boolean,
      default: false,
    },
    sortable: {
  ...
  data() {
    return {
      ...
      errorFiled: "",
      edit: { columnIndex: -1, rowIndex: -1 }, // 當前双擊編輯的行與列坐標
      editStatus: {},
      summary: false, // 是否顯示合計
      // 目前只支持從後台返回的summaryData數據
      summaryData: [],
      summaryIndex: {},
      remoteColumns: [], // 需要每次刷新或分頁後從後台加載字典數據源的列配置
      cellStyleColumns: {}, // 有背景顏色的配置
      fxRight: false, //是否有右邊固定表頭
      selectRows: [], //當前選中的行
      isChrome: false,
      //vol-table带數據源的單元格是否啟用tag標籤(下拉框等單元格以tag標籤顯示)
  ...
  methods: {
      ...
      this.http.post("/api/Sys_Dictionary/GetVueDictionary", keys).then((dic) => {
        dic.forEach((x) => {
          ...
          columnBind.forEach((c) => {
            ...
            // 轉換數據源的類型與列的類型一致(2020.04.04)
            else if (
              c.key == x.dicNo &&
              (c.valueType == "int" || c.valueType == "sbyte")
            ) {
              x.data.forEach((d) => {
                // 2020.09.01增加對數字類型的二次判斷
                if (!isNaN(d.key)) {
                  d.key = ~~d.key;
                }
              });
    ...
    rowClick(row, column, event) {
      ...
      if (this.clickEdit && this.edit.rowIndex != -1) {
        ...
        //當正在編輯，且點擊到其他行時，在原編輯的行结束編輯後，觸發新行的rowClick事件
        //正在編輯時，禁止出發rowClick事件
        if (this.edit.rowIndex == -1) {
          this.$emit("rowClick", { row, column, event });
        }
    ...
    getFilePath(pathSring, column) {
      ...
      if (!pathSring) return [];
      // 增加圖片自定義操作
      // 返回格式必須是[{name:"文件名",path:"圖片全路徑或base64格式"}]
      if (column.formatter) {
    ...
    rowEndEdit(row, column, event) {
      ...
      if (this.edit.rowIndex != -1) {
        //2022.06.26修複表格内容切換後行數不一致時不能編輯的問題
    ...
    validateColum(option, data) {
      ...
      if (editType == "int" || editType == "decimal" || editType == "number") {
        ...
        if (
          ...
          _errMsg = this.$ts("不能小於") + option.edit.min;
          return false;
        }
        if (
          ...
          _errMsg = this.$ts("不能大於") + option.edit.max;
          return false;
        }
        return true;
      }
    ...
    addRow(row) {
      ...
      this.columns.forEach((x) => {
        // 2022.05.06 添加行時，如果列有編輯屬性，設置開啟編輯(避免關閉編輯後，無法再次啟用編輯)??
        //x.readonly = false;
    ...
    formatter(row, column, template) {
      ...
      let source = column.bind.data.filter((x) => {
        ...
        // 2020.06.06修複單獨使用table组件時,key為數字0時轉換成文本失敗的問題
    ...
    getSelectFormatter(column, val) {
      ...
      for (let index = 0; index < valArr.length; index++) {
        (column.bind.orginData && column.bind.orginData.length
          ...
          // 2020.06.06修複數據源為selectList時,key為數字0時不能轉換文本的問題
    ...
    toNextCell(tableName, row, nextField, newRow) {
      //2024.07.05按回車鍵後自動跳轉到下一行指定字段並獲取焦點
      for (let index = 0; index < this.columns.length; index++) {
        const _column = this.columns[index];
        if (_column.edit) {
          if (!this.validateRow(row, _column)) {

<template>
    <div class="node-filter-container">
        <!-- <div class="add-btn">
          <span class="name">條件設置</span>  <el-button @click="addItem" link><i>+</i>添加字段</el-button>
        </div> -->
        ...
        <div>
            <table>
                ...
                <tr v-for="(item, index) in filters" :key="index">
                    ...
                    <td><el-select size="small" v-model="item.filterType" :placeholder="$ts('請選擇')">
                    ...
                    <td>
                        <template v-if="item.data">
                            ...
                            <el-select-v2 style="width: 100%;" v-else multiple size="small" :options="item.data"
                                v-model="item.value" :placeholder="$ts('請選擇')">
...
export default {
    ...
    data() {
        return {
            ...
            customSql: "",
            value: "",
            //{ field: "名稱", value: "", filterType: "=" },
            //  filters: [],
            fieldsOptions: [

            ],
            t: [],
            filterType: [{ name: "等於(=)", value: "=" },
            { name: "不等於(!=)", value: "!=" },
            { name: "大於(>)", value: ">" },
            { name: "大於等於(>=)", value: ">=" },
            { name: "小於(<)", value: "<" },

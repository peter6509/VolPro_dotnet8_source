<template>
  <vol-box v-model="model" :padding="30" title="修改密碼" :width="500" :height="250">
    ...
    <div>
      <el-input :placeholder="$ts('密碼')" v-model="password" size="large" style="width: 100%; margin-top: 15px" />
    </div>
    <template #footer>
  ...
  <vol-box v-model="modelAuth" :padding="5" :title="row.UserTrueName" :width="1200">
    <el-table :data="roleTree" style="width: 100%;height:calc(100vh - 150px)" row-key="id" border
      ...
      <el-table-column prop="text" :label="$ts('菜單')" width="250">
      </el-table-column>
  ...
  <vol-box v-model="modelUser" :padding="5" title="選擇數據" :width="1000">
    <div class="search-form">
      <label>帳號：</label> <el-input style="width: 150px;" v-model="userName" :placeholder="$ts('請輸入')"></el-input>
      <label class="label">姓名：</label> <el-input style="width: 150px;" v-model="userTrueName"
        :placeholder="$ts('請輸入')"></el-input>
    ...
    <template #footer>
      <div style="text-align: center;">
        ...
        <el-button type="primary" size="small" @click="selectClick">{{ $ts('確定') }}</el-button>
      </div>
    </template>
  </vol-box>
</template>
...
{ field: 'UserName', title: '帳號', type: 'string', width: 120, readonly: true, require: true, align: 'left', sort: true },
...
{ field: 'CreateDate', title: '注册時間', type: 'datetime', width: 150, readonly: true, align: 'left', sort: true },

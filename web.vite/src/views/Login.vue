  ...
  <div class="project-name"></div>
    ...
    <div class="login-form">
      ...
      <div class="loging-btn">
        <el-button
          ...
          long
        >
          <span v-if="!loading">{{ $ts('登錄') }}</span>
          <span v-else>{{ $ts('正在登錄') }}...</span>
        </el-button>
      </div>
            <!-- 帳號信息 -->
            <!-- <div class="account-info">
        <p>演示帳號：admin666 &nbsp; &nbsp;密碼:123456</p>
        <p>本地帳號：admin &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;密碼:123456</p>
        <p><a href="https://jq.qq.com/?_wv=1027&k=Sqstuy0M" style="text-decoration: none"
            target="_blank">QQ3群:743852316</a>
          &nbsp; &nbsp;&nbsp; &nbsp;
          <a href="http://v2.volcore.xyz/document/guide" style="text-decoration: none" target="_blank">框架文檔</a>
        </p>
      </div> -->
      <!-- 鏈接位置 -->
      <!-- <div class="app-link" >
        <a href="#" style="text-decoration: none">移動端掃碼</a>
        <a>
    ...
    <div class="login-footer">
      <a href="" style="text-decoration: none" target="blank"
        >SolidFocus 專案管理系統 V1.0版</a
      >
    </div> 

    <img class="login-bg" src="/static/project_bg.png" />
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const login = () => {
      if (!userInfo.userName) return $message.error($ts(['請輸入', '帳號']));
      if (!userInfo.password) return $message.error($ts(['請輸入', '密碼']));
      if (!userInfo.verificationCode) {
        return $message.error($ts(['請輸入', '驗證碼']));
      }
      loading.value = true;
      http
        .post('/api/user/login', userInfo, $ts('正在登錄') + '....')
        .then((result) => {
          if (!result.status) {
            loading.value = false;
            getVierificationCode();
